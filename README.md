...
3. ОКНО ГЕНЕРАЦИИ КАРТОЧЕК
 ...
 1.header: кнопка "Статистика" PHRASE WEAVER "Настройки"
     при нажатии на "Статистика" ,открывается окно со статистикой
     при нажатии на "Настройки" , отскрывается окно с настройками
     при нажатии на PHRASE WEAVER возвращаемся на главное окно
     ...
     поле для ввода перевода \либо на языке изучения (если вводится правильно менят цвет фона на зеленый, если нет - на красный)
      ...
      TODO :  статистика по карточкам ,сколько изучено ,сколько повторяемых, сколько в очереди на повторение, сколько в очереди на тренировку (с учетом кол-ва повторений в колоде)
      ...
      TODO :  выбор языков интерфейса, гланые русский и английский
```
...
1. мы сразу настраиваем docker для деплоя на render.com
2. после каждого шажочка пушим изменения на github в ветку dev-brunch , чтобы изменения деплоились на render.com
3. если возможно настроить автоматическое переключение энд поинтов для локального теста и деплоя на сервер render
4. используем самые свежие стабильные версии библиотек
погнали!